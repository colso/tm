...
def proc_run(magnet, host=None, t_id=None, t_port=None, t_pass=None):
	...
	try:
		...
		os.system('/root/bin/stopTM.sh')
		time.sleep(3)
		print("Try to start transmission serverice")
		os.system('/root/bin/startTM.sh')
		return {}, {}, max_cnt

	print(datetime.today())
	del_cnt = 0
	magnet_hash_code = magnet.split(':')[-1]
	try:
		for item in item_list:
	...
	if (delete_flag == 0) and ((len(item_list) - del_cnt) < max_cnt) and (not magnet in running_dic):
